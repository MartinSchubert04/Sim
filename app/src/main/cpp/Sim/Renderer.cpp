...
void Renderer::DrawArrow(Vector3 startPos, Vector3 endPos, float radius, float coneLen, float sides, Color color) {
  ...
  Vector3 coneStartPos = endPos;
  Vector3 coneEndPos = Vector3Add(endPos, Vector3Scale(dir, coneLen));

  DrawCylinderEx(startPos, endPos, radius, radius, 8, color);
  DrawCylinderEx(coneStartPos, coneEndPos, coneStartRadius, 0, 8, color);
}
...
void Renderer::DrawText3D(Font font, const char *text, Vector3 position, float fontSize, float fontSpacing,
  ...
  for (int i = 0; i < length;) {
    ...
    if (codepoint == '\n') {
      ...
      if (font.glyphs[index].advanceX == 0) textOffsetX +=
                                                    (float) font.recs[index].width * scale +
                                                    fontSpacing;
